...
class EmissionModel(object):
	...
	"""
	def __init__(self,number_of_states,dim=12,number_of_hmm_components=5):
		self.number_of_states = number_of_states 	# can change
		self.dim = dim 				# should be 12
		self.states = range(1,number_of_states+1)
		self._model = None
		self.number_of_hmm_components = number_of_hmm_components
	...
	def _train_chord_tones_hmm(self, X, y):
		...
		X_by_state, L_by_state = self.prepare_data(X, y)
		for state in self.states:
			hmm_i = hmm.MultinomialHMM(n_components=self.number_of_hmm_components)
			hmm_i.fit(X_by_state[state], L_by_state[state])
	...
	def prepare_data(self, X, y):
		...
		for i, song in enumerate(y):
			for j, chord in enumerate(song):
				...
				one_hot_vectors = []
				for note in X[i][j]:
					one_hot_note = to_one_hot_vector(note, self.dim)
					length += 1
					one_hot_vectors.append(one_hot_note)
				if len(one_hot_vectors) > 0:
					X_by_state[chord] += one_hot_vectors
					L_by_state[chord].append(length)
		for state in self.states:
			X_by_state[state] = np.asarray(X_by_state[state])
			L_by_state[state] = np.asarray(L_by_state[state])
		return X_by_state, L_by_state
	...
	def logprob_chord_tones_hmm(self,state,obv):
		X = []
		for note in obv:
			X.append(to_one_hot_vector(note, self.dim))
		if len(X) == 0:
			return 0
		X = np.asarray(X)
		return self.chord_hmms[state].score(X)
