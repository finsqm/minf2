...
def transpose_to_key_from_c(tpc_t, key_raw):
    key_pc = process_chord_string(key_raw)
    tpc = (tpc_t + key_pc) % 12
    return tpc
    ...
    def load(self, filename='by_note_cleaned.csv'):
        ...
        with open(data_path) as f:
            ...
            for row in reader:
                ...
                chord = process_chord_string(chord_raw)
                tpc = int(tpc_raw)
                # tpc_vec = to_one_hot_vector(tpc, TPC_DIM)
                tpc_original_key = transpose_to_key_from_c(tpc, key_raw)
                x.append(to_one_hot_vector(tpc_original_key, 12))
                y.append(to_one_hot_vector(chord, 12))
